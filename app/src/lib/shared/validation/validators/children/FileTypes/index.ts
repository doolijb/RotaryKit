...
type FileTypeArgs =
	| { fileTypes: FileType[]; extensions: FileExtension[] }
	| { fileTypes: FileType[] }
...
export class FileTypes extends Validator {
	...
	test = async ({ key, data }) => {
		...
		const res = files.every((file) => {
			...
			if (
				"fileTypes" in this.args &&
				!this.args.fileTypes.some((fileType: FileType) =>
					(fileTypes[fileType] as string[])?.includes(extension)
				)
			) {
				return false
			}
	...
	listExtensions: () => string = () => {
		...
		if ("fileTypes" in this.args) {
			extensions = extensions.concat(
				...this.args.fileTypes.map((fileType) => fileTypes[fileType] ?? [])
			)
		}
