 ...
 */
export const usersToAdminRoles: PgTableWithColumns<any> & {usePermissions?: boolean} = pgTable("users_to_admin_roles", {
...
export const usersToAdminRoleRelations = relations(usersToAdminRoles, ({ one: One }) => ({
    user: One(users, {
        fields: [usersToAdminRoles.userId], references: [users.id]
    }),
    adminRole: One(adminRoles, {
        fields: [usersToAdminRoles.adminRoleId], references: [adminRoles.id]
    }),
}))
