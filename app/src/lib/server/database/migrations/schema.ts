...
export const adminPermissions = pgTable(
	...
	(table) => {
		return {
			uniqueActionResource: uniqueIndex("unique_action_resource").using(
				"btree",
				table.method.asc().nullsLast(),
...
export const users = pgTable(
	...
	(table) => {
		return {
			uniqueUsernames: uniqueIndex("unique_usernames").using(
				"btree",
...
export const emails = pgTable(
	...
	(table) => {
		return {
			uniqueUserPrimary: uniqueIndex("unique_user_primary")
				.using("btree", table.userId.asc().nullsLast(), table.isUserPrimary.asc().nullsLast())
				.where(sql`((is_user_primary = true) AND (user_id IS NOT NULL))`),
...
export const passphraseResets = pgTable(
	"passphrase_resets",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
...
export const passphrases = pgTable(
	...
	(table) => {
		return {
			uniqueUserPassphrases: uniqueIndex("unique_user_passphrases").using(
				"btree",
...
export const usersToAdminRoles = pgTable(
	...
	(table) => {
		return {
			...
			usersToAdminRolesUserIdAdminRoleIdPk: primaryKey({
				columns: [table.userId, table.adminRoleId],
...
export const adminRolesToPermissions = pgTable(
	...
	(table) => {
		return {
			...
			adminRolesToPermissionsAdminPermissionIdAdminRoleIdPk: primaryKey({
				columns: [table.adminPermissionId, table.adminRoleId],
