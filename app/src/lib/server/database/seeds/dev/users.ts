...
async function create({tx, user, email, passphrase}: UserSeedData) {
    ...
    if (!await providers.users.exists({tx, ...user})) {
        const results: [{id: UUID}] = await providers.users.create({tx, ...user, returning: {id: schema.users.id}})
        ...
        if (!await providers.emails.exists({tx, address: email.address})) {
            ...
            db.update(schema.emails).set({ userId: userId, ...email }).where(eq(schema.emails.address, email.address))
        }
    } else{
