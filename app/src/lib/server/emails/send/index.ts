...
export async function send<T extends React.ComponentType<any>>({
    ...
    for (const toEmailAddress of toEmailAddresses) {
        ...
        }).from(schema.emails).where(eq(schema.emails.address, toEmailAddress)).limit(1).then(async (rows) => {
            // Assign the email id and user id to the log
            if (rows.length > 0) {
                emailLog.recipientEmailId = rows[0].id
                emailLog.recipientUserId = rows[0].userId
                // Get the user's username
                await db.select({id: schema.users.id, username: schema.users.username}).from(schema.users).where(eq(schema.users.id, rows[0].userId)).limit(1).then((rows) => {
                    if (rows.length > 0) {
