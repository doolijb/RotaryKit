...
export async function checkObjectStorage() {
    ...
    if (!STORAGE_ACCESS_KEY_ID || !STORAGE_SECRET_ACCESS_KEY || !STORAGE_DEFAULT_REGION || !STORAGE_DEFAULT_BUCKET || !STORAGE_PRIVATE_ENDPOINT) {
    ...
    try {
        ...
        logger.error({ message: `Invalid URL for STORAGE_PRIVATE_ENDPOINT: ${STORAGE_PRIVATE_ENDPOINT}`, error })
        process.exit(1)
    }
