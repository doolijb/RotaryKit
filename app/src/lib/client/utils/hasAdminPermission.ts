...
export function hasAdminPermission({
	...
	user: SelectUser & {
		emails?: Partial<SelectEmail>[]
		toAdminRoles?: Partial<SelectUsersToAdminRoles>[]
		profileImages?: Partial<SelectImage>[]
		images?: Partial<SelectImage>[]
	}
	...
	for (const resource of resources) {
		...
		for (const adminPermission of adminPermissions) {
			...
			if (
				...
				adminPermission.resource === lodash.snakeCase(resource)
			) {
				return true
			}
		}
